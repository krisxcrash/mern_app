import React, {Component} from 'react';

class Module2 extends Component {
    render() {
        return (
            <div className="container">
                <div className="row module-one-div" id="module2">
                    <div className="col-md-6 col-xs-12">
                        <div className="square"></div>
                    </div>
                    <div className="col-md-6 col-xs-12">
                    <div className="row">
                        <div className="text-section-2">
                            <h4 className="module-title">PROJECT</h4>
                            <p className="module-p">
                            Lorem ipsum dolor amet echo park retro. Hashtag unicorn vegan, vinyl brunch glossier man bun distillery hoodie enamel pin semiotics shaman shoreditch 90's hell of.
                            </p>
                            <p><a href="www.google.com" className="learn-more-link"><span className="learn-span">LEAR</span>N MORE</a></p>
                        </div>
                    </div>
                    <div className="row">
                        <div className="col-md-12">
                        <h5 className="module-2-h5">02.</h5>
                        </div>
                    </div>
                    </div>
                </div>
            </div>

        )
    }
}

export default Module2;